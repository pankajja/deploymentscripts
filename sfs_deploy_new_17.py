...
def configureSfsComponent(comp):
    ...
    Reads all template files listed in config.json, replaces
    ...
    # Validate templateFiles key exists
    if 'templateFiles' not in configData:
        log('DEBUG', 'ERROR: templateFiles key not found in ' + compConfigJson)
        sys.exit(1)

    # Create output directory
    createFolder(compOutputDir)

    # Step b: Loop over all template files listed in config.json
    for templateFileName in configData['templateFiles']:
        ...
        # Step d: Validate param list exists in config.json
        if templateFileName not in configData:
            log('DEBUG', 'ERROR: No param list found for ' + templateFileName
                + ' in ' + compConfigJson)
            sys.exit(1)

        # Step e: Replace dot-notation placeholders using envInfo
        #
        ...
        # Step f: Write configured output file
