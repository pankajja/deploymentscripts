...
def log(logLevel, cmd):
    ...
    print(timeSuffix)
    print(str(timeSuffix) + ': ' + logLevel + ': ' + cmd)
...
def sfsDeploy():
    ...
    Core deployment — mirrors reference deploy() pattern:
      ...
      2. Extract tar    — to applRoot/SFS-<release>/
      3. os.symlink     — applRoot/SFS -> applRoot/SFS-<release>
      4. fixPermissions()
    """
    log('INFO', 'deploy()')
    clean()
    ...
    sfsReleaseDir = 'SFS-' + pa.release                    # SFS-0.1.0
    tarFileName   = sfsReleaseDir + '.tar.gz'              # SFS-0.1.0.tar.gz
    sfsTargetDir  = pa.applRoot + sfsReleaseDir            # applRoot/SFS-0.1.0
    sfsLink       = pa.applRoot + 'SFS'                    # applRoot/SFS

    # Step 1: Verify tar exists in staging directory
    log('INFO', 'Checking tar artifact: ' + tarFileName)
    checkFileExists('./' + tarFileName)

    # Step 2: Create release directory and extract tar
    os.makedirs(sfsTargetDir)
    log('INFO', 'Extracting ' + tarFileName + ' to ' + pa.applRoot)
    cmd = 'tar -xf ' + tarFileName + ' -C ' + pa.applRoot
    status, result = executeCommand(cmd)
    if status != 0:
        log('DEBUG', 'ERROR: Failed to extract tar: ' + tarFileName)
        sys.exit(1)
    log('INFO', 'Extraction successful: ' + sfsTargetDir)

    # Step 3: Verify expected component directories inside extracted tar
    components = getComponents()
    for comp in components:
    ...
    # Step 4: Create symlink applRoot/SFS -> applRoot/SFS-<release>
...
def validate():
    """
    Validate deployment — check all expected files and
    directories are in place after deployment.
    Mirrors reference code validate() pattern.
    """
    log('INFO', 'validate()')

    components = getComponents()

    # Validate SFS release directory
    sfsTargetDir = pa.applRoot + 'SFS-' + pa.release
    if not os.path.exists(sfsTargetDir):
        print('ERROR: ' + sfsTargetDir + ' is not deployed. Please Check')
        sys.exit(1)

    # Validate SFS symlink
    sfsLink = pa.applRoot + 'SFS'
    if not os.path.exists(sfsLink):
        print('ERROR: ' + sfsLink + ' symlink does not exist. Please Check')
        sys.exit(1)

    # Validate config release directory
    configTargetDir = pa.applRoot + 'config-' + pa.release
    if not os.path.exists(configTargetDir):
        print('ERROR: ' + configTargetDir + ' is not deployed. Please Check')
        sys.exit(1)

    # Validate config symlink
    configLink = pa.applRoot + 'config'
    if not os.path.exists(configLink):
        print('ERROR: ' + configLink + ' symlink does not exist. Please Check')
        sys.exit(1)

    # Validate each component directory and application.properties
    for comp in components:
        compSfsDir    = sfsTargetDir    + '/' + comp
        compConfigDir = configTargetDir + '/' + comp
        appProperties = compConfigDir   + '/application.properties'

        for path in [compSfsDir, compConfigDir, appProperties]:
            if not os.path.exists(path):
                print('ERROR: ' + path + ' is not deployed. Please Check')
                sys.exit(1)

    print('INFO: Successfully validated the SFS deployment')
