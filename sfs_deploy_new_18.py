...
def getEnvInfo(comp, category, name):
    """Safe accessor for 3-level nested envInfo keys — returns None if not found.
    envInfo[comp][category][name] e.g. envInfo['tfg']['server']['port']
    """
    global envInfo
    if envInfo is None:
        return None
    if comp in envInfo and category in envInfo[comp] and name in envInfo[comp][category]:
        return envInfo[comp][category][name]
    return None
...
def configureSfsComponent(comp):
    ...
    for templateFileName in configData.keys():
        ...
        for param in configData[templateFileName]:
            ...
            # Safe 3-level lookup — envInfo[comp][category][name]
            value = getEnvInfo(comp, category, name)
            if value is None:
                log('DEBUG', 'ERROR: No value found in envInfo for param: '
                    + param + ' (comp=' + comp + ', category=' + category + ', name=' + name + ')')
                sys.exit(1)
            print(param, value)
