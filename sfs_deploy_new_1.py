...
def sfsDeploy():
    ...
    Core deployment — mirrors reference deploy() pattern:
      ...
      3. Find inner .tar     — one per component e.g. TFG-0.1.0.tar, PCT-0.1.0.tar
    ...
    # Step 3: Find all inner .tar files inside temp extract folder
    innerTars = [f for f in os.listdir(tempExtractDir) if f.endswith('.tar')]
    if not innerTars:
    ...
    for innerTar in innerTars:
        innerTarPath = tempExtractDir + '/' + innerTar
        log('INFO', 'Extracting inner tar: ' + innerTar + ' to ' + sfsTargetDir)
        cmd = 'tar -xf ' + innerTarPath + ' -C ' + sfsTargetDir
        status, result = executeCommand(cmd)
        if status != 0:
