...
def configureSfsComponent(comp):
    ...
    # Paths — both config.json and template live in our staging repo
    compConfigJson   = './config/' + comp + '/config.json'                          # staging repo
    templateFilePath = './config/' + comp + '/application.template.properties'     # staging repo
...
def sfsDeploy():
    ...
    Core deployment — mirrors reference deploy() pattern:
      ...
      5. Cleanup tmp_extract — guaranteed via try/finally (success or failure)
      6. os.symlink          — applRoot/SFS -> applRoot/SFS-<release>
      7. fixPermissions()
    """
    log('INFO', 'deploy()')
    clean()
    ...
    sfsLink        = pa.applRoot + 'sfs-app'                 # applRoot/sfs-app
    tempExtractDir = stagingDir + '/tmp_extract'             # temp staging area for outer tar
    ...
    # Remove if exists from previous run to avoid stale content
    if os.path.exists(tempExtractDir):
        shutil.rmtree(tempExtractDir)
    createFolder(tempExtractDir)

    try:
        # Step 3: Extract outer tar into temp dir
        ...
        # Step 4: Recursively search for all inner .tar files inside temp extract folder
        ...
        # Step 5: Create SFS release directory if not exists, then extract each inner tar into it
    ...
    # Step 6: For each component — check what tar extracted
    # IF tfg/     extracted → rename to tfg-0.1.0  then create symlink
    # IF tfg-0.1.0/ extracted → skip rename         then create symlink forcefully (ln -fs)
    components = getComponents()
    for comp in components:
        ...
        elif os.path.isdir(compDir):
            # tfg/ extracted — rename to tfg-0.1.0/
    ...
    # Step 7: Create symlink applRoot/sfs-app -> applRoot/sfs-app-<release>
...
def init():
    ...
    parser.add_argument('--env',       nargs='?', help='Environment e.g. cit01 (required for config/all deploy)',
    ...
    parser.add_argument('--envFolder', nargs='?', help='Path to directory containing env.json (required for config/all deploy)',
