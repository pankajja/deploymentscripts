#!/usr/bin/python3

import json, sys, fileinput, datetime, time
import os, shutil, collections
import argparse, socket, getpass
import subprocess, inspect
import logging

from pprint import pprint

# ─────────────────────────────────────────────
# Startup — change working dir to script's own
# staging directory so all relative paths
# (tar, config/) resolve correctly regardless
# of where the script is called from
# ─────────────────────────────────────────────
stagingDir = os.path.dirname(os.path.realpath(__file__))
os.chdir(stagingDir)

localHostName = socket.gethostname()

# echoVal: set to "echo" for dry-run/debug mode, "" for live
echoVal = ""

# Global state
pa      = None
envInfo = None
logger  = None
logFile = None

# Log directory
LOG_DIR = '/home/claude/applroot/logs'

# Known components — always use base names without version
# e.g. 'tfg' not 'tfg-0.1.0' — version is appended by the code
KNOWN_COMPONENTS = ['tfg', 'pct', 'su']


# ─────────────────────────────────────────────
# Logging Setup
# ─────────────────────────────────────────────

def setupLogging():
    """
    Set up logging with:
    - File handler: detailed logs → /cls/appl/logs/sfs_deploy_YYYYMMDD_HHMMSS.log
    - Console handler: minimal INFO only
    """
    global logger

    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    logFile   = LOG_DIR + '/sfs_deploy_' + timestamp + '.log'

    # Create log dir if not exists
    if not os.path.exists(LOG_DIR):
        os.makedirs(LOG_DIR)

    logger = logging.getLogger('sfs_deploy')
    logger.setLevel(logging.DEBUG)

    # File handler — DEBUG + INFO (full detail)
    fh = logging.FileHandler(logFile)
    fh.setLevel(logging.DEBUG)
    fh.setFormatter(logging.Formatter('%(asctime)s: %(levelname)s: %(message)s', '%Y-%m-%d %H:%M:%S'))

    # Console handler — WARNING and above only (suppressed for milestone use)
    ch = logging.StreamHandler()
    ch.setLevel(logging.WARNING)
    ch.setFormatter(logging.Formatter('%(asctime)s: %(levelname)s: %(message)s', '%Y-%m-%d %H:%M:%S'))

    logger.addHandler(fh)
    logger.addHandler(ch)

    log('INFO', 'Log file: ' + logFile)
    return logFile


def console(msg):
    """Print minimal milestone messages to console only."""
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(timestamp + ': ' + msg)


# ─────────────────────────────────────────────
# Utility Functions
# ─────────────────────────────────────────────

def readJsonData(jsonFile):
    """Check file exists then read and parse JSON."""
    checkFileExists(jsonFile)
    log('DEBUG', 'Reading: ' + jsonFile)
    with open(jsonFile) as dataFile:
        jsonData = json.load(dataFile)
    return jsonData


def log(logLevel, msg):
    """
    Timestamped logger:
    - Console: INFO and above only
    - File: DEBUG and above (detailed)
    """
    global logger
    if logger is None:
        # fallback before logging is setup
        timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print(str(timestamp) + ': ' + logLevel + ': ' + msg)
        return
    if logLevel == 'DEBUG':
        logger.debug(msg)
    elif logLevel == 'INFO':
        logger.info(msg)
    elif logLevel == 'ERROR':
        logger.error(msg)
    else:
        logger.info(msg)


def executeCommand(cmd):
    """
    Run a shell command using getstatusoutput.
    If echoVal is set, prepend it (e.g. 'echo') for dry-run mode.
    Returns (status, result).
    """
    if echoVal != "":
        cmd = echoVal + " " + cmd

    log('DEBUG', cmd)
    (status, result) = subprocess.getstatusoutput(cmd)
    log('DEBUG', 'Command Status: ' + str(status))
    if result is not None and len(result) > 0:
        log('DEBUG', 'Console Output: ' + ''.join(result))
    return status, result


def createFolder(folder):
    """Create a directory (mkdir -p) if it does not exist."""
    if not os.path.exists(folder):
        log('DEBUG', 'Creating ' + folder)
        cmd = 'mkdir -p ' + folder
        executeCommand(cmd)


def checkFileExists(fileName):
    """Exit with error if a required file is missing."""
    if not os.path.exists(fileName):
        log('DEBUG', 'ERROR: ' + fileName + " doesn't exist. Please refer to the deployment guide")
        sys.exit(1)


def getEnvInfo(category, name):
    """Safe accessor for 2-level nested envInfo keys — returns None if not found.
    envInfo[category][name] e.g. envInfo['tfgtls']['keyName']
    """
    global envInfo
    if envInfo is None:
        return None
    if category in envInfo and name in envInfo[category]:
        return envInfo[category][name]
    return None


# ─────────────────────────────────────────────
# Component Resolution
# ─────────────────────────────────────────────

def getComponents():
    """
    Resolve the list of components to deploy.
    --component all      -> all known components [tfg, pct]
    --component tfg      -> ['tfg']
    --component pct tfg  -> ['pct', 'tfg']
    """
    if pa.component is None or pa.component == ['all']:
        return KNOWN_COMPONENTS
    # Normalise to lowercase to match directory names tfg / pct
    return [c.lower() for c in pa.component]


# ─────────────────────────────────────────────
# Clean  (aligned with reference clean())
# ─────────────────────────────────────────────

def clean():
    """
    Remove existing release folder and symlink before fresh deploy.
    Mirrors reference code clean() pattern.
    """
    log('INFO', 'clean()')

    sfsReleaseFolder = pa.applRoot + 'sfs-app-' + pa.release   # applRoot/sfs-app-0.1.0
    sfsLink          = pa.applRoot + 'sfs-app'                 # applRoot/sfs-app
    configFolder     = pa.applRoot + 'sfsconfig-' + pa.release # applRoot/sfsconfig-0.1.0
    configLink       = pa.applRoot + 'sfsconfig'               # applRoot/sfsconfig

    # Remove sfs-app symlink if exists
    if os.path.exists(sfsLink):
        cmd = 'rm -rf ' + sfsLink
        executeCommand(cmd)

    # Remove sfs-app release folder if exists
    if os.path.exists(sfsReleaseFolder):
        cmd = 'rm -rf ' + sfsReleaseFolder
        executeCommand(cmd)

    # Remove config symlink if exists
    if os.path.exists(configLink):
        cmd = 'rm -rf ' + configLink
        executeCommand(cmd)

    # Remove config release folder if exists
    if os.path.exists(configFolder):
        cmd = 'rm -rf ' + configFolder
        executeCommand(cmd)


# ─────────────────────────────────────────────
# Configure  (aligned with reference configure())
# ─────────────────────────────────────────────

def configure():
    """
    Config deployment — for each component:
      a. Read config/<COMP>/config.json from staging repo
      b. Read application.template.properties from extracted SFS dir
      c. Replace dot-notation placeholders with values from env.json
      d. Write application.properties -> applRoot/config-<release>/<COMP>/
    Mirrors reference code configure() -> configuretradeapi() pattern.
    """
    log('INFO', 'configure()')

    components = getComponents()
    for comp in components:
        configureSfsComponent(comp)


def configureSfsComponent(comp):
    """
    Configure a single SFS component.
    Reads all template files defined as keys in config.json, replaces
    dot-notation placeholders and writes configured output files.
    config.json structure:
    {
        "application.template.properties": [ "env.applRoot", ... ],
        "SQLJDBCDriver.template.conf":      [ "db.keyTabFilePath", ... ]
    }
    """
    log('INFO', 'configureSfsComponent: ' + comp)

    configReleaseDir = 'sfsconfig-' + pa.release                         # sfsconfig-0.1.0
    configTargetDir  = pa.applRoot + configReleaseDir                 # applRoot/sfsconfig-0.1.0
    compOutputDir    = configTargetDir + '/' + comp                   # applRoot/sfsconfig-0.1.0/pct

    # Resolve config folder — try base name first e.g. config/pct/
    # if not found try versioned name e.g. config/pct-0.1.0/
    compConfigDir = './config/' + comp
    if not os.path.exists(compConfigDir):
        compConfigDir = './config/' + comp + '-' + pa.release
        if not os.path.exists(compConfigDir):
            log('DEBUG', 'ERROR: config folder not found for component: ' + comp
                + ' — tried ./config/' + comp + ' and ./config/' + comp + '-' + pa.release)
            sys.exit(1)
    log('INFO', 'Using config folder: ' + compConfigDir)

    compConfigJson = compConfigDir + '/config.json'

    # Step a: Read config.json from staging repo
    log('INFO', 'Reading config.json: ' + compConfigJson)
    configData = readJsonData(compConfigJson)

    # Create output directory
    createFolder(compOutputDir)

    # Step b: Loop over all keys in config.json
    # Each key is a template filename e.g. application.template.properties
    for templateFileName in configData.keys():

        templateFilePath = compConfigDir + '/' + templateFileName

        # Derive output file name — remove .template from name
        # e.g. application.template.properties → application.properties
        #      SQLJDBCDriver.template.conf      → SQLJDBCDriver.conf
        outputFileName = templateFileName.replace('.template', '')
        outputFilePath = compOutputDir + '/' + outputFileName

        # Step c: Read template file
        log('INFO', 'Reading template file: ' + templateFilePath)
        checkFileExists(templateFilePath)
        with open(templateFilePath) as f:
            fileData = f.read()
        f.close()

        # Step d: Replace dot-notation placeholders using envInfo
        #
        #   config.json lists params like "env.applRoot", "coredb.dbPort"
        #   Split on '.' -> category = "env",    name = "applRoot"
        #                -> envInfo[comp]["env"]["applRoot"] = "/cls/appl"
        #   Replace "env.applRoot" in template with "/cls/appl"
        #
        log('INFO', 'Replacing placeholders for: ' + templateFileName)
        for param in configData[templateFileName]:
            category = param.split('.')[0]    # e.g. "env"
            name     = param.split('.')[1]    # e.g. "applRoot"

            # Safe 2-level lookup — envInfo[category][name]
            value = getEnvInfo(category, name)
            if value is None:
                log('DEBUG', 'ERROR: No value found in envInfo for param: '
                    + param + ' (category=' + category + ', name=' + name + ')')
                sys.exit(1)
            log('DEBUG', 'Replacing: ' + param + ' -> ' + value)
            fileData = fileData.replace(param, value)

        # Step e: Write configured output file
        log('INFO', 'Configuring ' + outputFilePath)
        with open(outputFilePath, 'w') as f:
            f.write(fileData)
        f.close()
        log('INFO', 'Generated: ' + outputFilePath)

    log('INFO', 'Component ' + comp + ' configured successfully.')


# ─────────────────────────────────────────────
# Fix Permissions  (aligned with reference fixPermissions())
# ─────────────────────────────────────────────

def fixPermissions():
    """
    Apply chmod and chown on deployed folders that exist.
    Mirrors reference code fixPermissions() pattern.
    """
    log('INFO', 'fixPermissions()')

    allFolders = [
        pa.applRoot + 'sfs-app-'   + pa.release,   # applRoot/sfs-app-0.1.0
        pa.applRoot + 'sfsconfig-' + pa.release,   # applRoot/sfsconfig-0.1.0
    ]

    # Only act on folders that actually exist at this point
    folders = [f for f in allFolders if os.path.exists(f)]

    if not folders:
        log('DEBUG', 'No folders to apply permissions on.')
        return

    # Apply permissions
    cmd = 'chmod -R 750 ' + ' '.join(folders)
    executeCommand(cmd)

    # Apply ownership
    cmd = 'chown -R ' + getpass.getuser() + ' ' + ' '.join(folders)
    executeCommand(cmd)


# ─────────────────────────────────────────────
# Core Deployment  (--type core)
# ─────────────────────────────────────────────

def sfsDeploy():
    """
    Core deployment — handles 2 levels of tar extraction:
      Level 1: sfs-app-apo-test-1.1.tar      (outer tar)
      Level 2: sfs-tfg-apo-test-1.1.tar      (component tar — directly contains tfg/ or pct-0.1.0/)

      1. clean()           — remove old release + symlinks
      2. Extract Level 1   — to tmp_extract
      3. Find Level 2 tars — sfs-tfg-*.tar, sfs-pct-*.tar
      4. Extract Level 2   — directly into applRoot/sfs-app-<release>/
      5. Cleanup           — guaranteed via try/finally
      6. Rename + symlink  — tfg -> tfg-0.1.0, pct -> pct-0.1.0
      7. Create top symlink — applRoot/sfs-app -> applRoot/sfs-app-<release>
      8. fixPermissions()
    """
    log('INFO', 'deploy()')
    clean()

    sfsReleaseDir  = 'sfs-app-' + pa.release                 # sfs-app-0.1.0
    tarFileName    = sfsReleaseDir + '.tar'                   # sfs-app-0.1.0.tar
    tarFilePath    = pa.releasePath + '/' + tarFileName       # /cls/release/sfs-app-0.1.0.tar
    sfsTargetDir   = pa.applRoot + sfsReleaseDir              # applRoot/sfs-app-0.1.0
    sfsLink        = pa.applRoot + 'sfs-app'                  # applRoot/sfs-app
    tempExtractDir = stagingDir + '/tmp_extract'              # temp staging area

    # Step 1: Verify outer tar exists in release directory
    log('INFO', 'Checking tar artifact: ' + tarFilePath)
    checkFileExists(tarFilePath)

    # Step 2: Create temp dir — remove stale content from previous run
    if os.path.exists(tempExtractDir):
        shutil.rmtree(tempExtractDir)
    createFolder(tempExtractDir)

    try:
        # Step 3: Extract Level 1 — outer tar into temp dir
        log('INFO', 'Extracting Level 1 tar: ' + tarFilePath)
        cmd = 'tar -xf ' + tarFilePath + ' -C ' + tempExtractDir
        status, result = executeCommand(cmd)
        if status != 0:
            log('DEBUG', 'ERROR: Failed to extract Level 1 tar: ' + tarFileName)
            sys.exit(1)
        log('INFO', 'Level 1 extraction successful')

        # Step 4: Find Level 2 tars — sfs-tfg-apo-test-1.1.tar, sfs-pct-apo-test-1.1.tar
        level2Tars = []
        for root, dirs, files in os.walk(tempExtractDir):
            for f in files:
                if f.endswith('.tar'):
                    level2Tars.append(os.path.join(root, f))

        if not level2Tars:
            log('DEBUG', 'ERROR: No Level 2 .tar files found inside ' + tarFileName)
            sys.exit(1)
        log('INFO', 'Found Level 2 tar files: ' + str(level2Tars))

        # Step 5: Create SFS release directory if not exists
        if not os.path.exists(sfsTargetDir):
            os.makedirs(sfsTargetDir)

        # Step 6: Extract each Level 2 tar directly into sfsTargetDir
        # Level 2 tars directly contain tfg/ or pct-0.1.0/ — no Level 3 tar
        for level2Tar in level2Tars:
            log('INFO', 'Extracting Level 2 tar: ' + level2Tar + ' to ' + sfsTargetDir)
            cmd = 'tar -xf ' + level2Tar + ' -C ' + sfsTargetDir
            status, result = executeCommand(cmd)
            if status != 0:
                log('DEBUG', 'ERROR: Failed to extract Level 2 tar: ' + level2Tar)
                sys.exit(1)
            log('INFO', 'Level 2 tar extracted successfully: ' + level2Tar)

    finally:
        # Always cleanup temp folder — whether deployment succeeds or fails
        if os.path.exists(tempExtractDir):
            log('INFO', 'Cleaning up temp folder: ' + tempExtractDir)
            shutil.rmtree(tempExtractDir)
            log('INFO', 'Temp folder cleaned up successfully')

    # Step 9: For each component — check what Level 2 tar extracted
    # IF tfg/       extracted → rename to tfg-<release>  then create symlink
    # IF tfg-<any>/ extracted → skip rename               then create symlink
    components = getComponents()
    for comp in components:
        compDir        = sfsTargetDir + '/' + comp                    # applRoot/sfs-app-1.1/tfg
        compVersionDir = sfsTargetDir + '/' + comp + '-' + pa.release # applRoot/sfs-app-1.1/tfg-1.1
        compLink       = sfsTargetDir + '/' + comp                    # applRoot/sfs-app-1.1/tfg (symlink)

        # Check if any versioned directory starting with comp- exists
        # e.g. pct-0.1.0/ when release is 1.1
        existingVersionDir = None
        if os.path.exists(compVersionDir):
            existingVersionDir = compVersionDir
        else:
            # Look for any dir starting with comp- in sfsTargetDir
            for item in os.listdir(sfsTargetDir):
                itemPath = sfsTargetDir + '/' + item
                if item.startswith(comp + '-') and os.path.isdir(itemPath) and not os.path.islink(itemPath):
                    existingVersionDir = itemPath
                    break

        if existingVersionDir:
            # Versioned dir already exists after extraction — skip rename
            log('INFO', 'Versioned directory already exists after extraction, skipping rename: ' + existingVersionDir)
            compVersionDir = existingVersionDir   # use actual versioned dir for symlink
        elif os.path.isdir(compDir) and not os.path.islink(compDir):
            # tfg/ extracted as real directory — rename to tfg-<release>/
            log('INFO', 'Renaming ' + compDir + ' to ' + compVersionDir)
            os.rename(compDir, compVersionDir)
            # Verify rename was successful
            if not os.path.exists(compVersionDir):
                log('DEBUG', 'ERROR: Rename failed — versioned directory not found: ' + compVersionDir)
                sys.exit(1)
            log('INFO', 'Verified versioned directory: ' + compVersionDir)
        else:
            log('DEBUG', 'ERROR: Neither ' + compDir + ' nor versioned directory found after extraction')
            sys.exit(1)

        # Create symlink forcefully — ln -fs (works whether symlink exists or not)
        log('INFO', 'Creating component symlink: ' + compLink + ' -> ' + compVersionDir)
        cmd = 'ln -fs ' + compVersionDir + ' ' + compLink
        executeCommand(cmd)

    # Step 10: Create sfs-app symlink — remove existing first then create fresh
    log('INFO', 'Creating symlink: ' + sfsLink + ' -> ' + sfsTargetDir)
    if os.path.islink(sfsLink):
        log('INFO', 'Removing existing symlink: ' + sfsLink)
        os.unlink(sfsLink)
    cmd = 'ln -s ' + sfsTargetDir + ' ' + sfsLink
    executeCommand(cmd)

    fixPermissions()

    log('INFO', '--- SFS Core Deployment Completed Successfully ---')



# ─────────────────────────────────────────────
# Config Deployment  (--type config)
# ─────────────────────────────────────────────

def sfsConfigDeploy():
    """
    Config deployment — mirrors reference deploy() + configure() pattern:
      1. clean()        — remove old config release + symlink
      2. configure()    — replace placeholders, write application.properties
      3. os.symlink     — applRoot/config -> applRoot/config-<release>
      4. fixPermissions()
    """
    log('INFO', 'sfsConfigDeploy()')

    configReleaseDir = 'sfsconfig-' + pa.release              # sfsconfig-0.1.0
    configTargetDir  = pa.applRoot + configReleaseDir          # applRoot/sfsconfig-0.1.0
    configLink       = pa.applRoot + 'sfsconfig'              # applRoot/sfsconfig

    # Step 1: Verify SFS extracted directory exists
    sfsTargetDir = pa.applRoot + 'sfs-app-' + pa.release
    checkFileExists(sfsTargetDir)

    # Step 2: Create config release directory if not exists
    if not os.path.exists(configTargetDir):
        os.makedirs(configTargetDir)

    # Step 3: Configure all components
    configure()

    # Step 4: Create config symlink — remove existing first then create fresh
    log('INFO', 'Creating symlink: ' + configLink + ' -> ' + configTargetDir)
    if os.path.islink(configLink):
        log('INFO', 'Removing existing symlink: ' + configLink)
        os.unlink(configLink)
    cmd = 'ln -s ' + configTargetDir + ' ' + configLink
    executeCommand(cmd)

    fixPermissions()

    log('INFO', '--- SFS Config Deployment Completed Successfully ---')


# ─────────────────────────────────────────────
# DB Utils Deployment  (--type dbutils)
# ─────────────────────────────────────────────

def sfsDbUtilsDeploy():
    """
    DB Utils deployment:
      1. Verify sfs-db-utils-<release>.tar exists in releasePath
      2. Create dbutilRoot directory if not exists
      3. Extract tar to dbutilRoot
      4. Verify sfsdb_utils-<release> dir exists after extraction
      5. Create symlink dbutilRoot/sfsdbutils -> dbutilRoot/sfsdb_utils-<release>
    """
    log('INFO', 'sfsDbUtilsDeploy()')

    tarFileName    = 'sfs-db-utils-' + pa.release + '.tar'           # sfs-db-utils-0.2.1.tar
    tarFilePath    = pa.releasePath + '/' + tarFileName               # /cls/release/sfs-db-utils-0.2.1.tar
    extractedDir   = pa.dbutilRoot + '/sfsdb_utils-' + pa.release    # /cls/dbutils/sfsdb_utils-0.2.1
    dbUtilsLink    = pa.dbutilRoot + '/sfsdbutils'                    # /cls/dbutils/sfsdbutils

    # Step 1: Verify tar exists in release directory
    log('INFO', 'Checking tar artifact: ' + tarFilePath)
    checkFileExists(tarFilePath)

    # Step 2: Create dbutilRoot directory if not exists
    createFolder(pa.dbutilRoot)

    # Step 3: Extract tar to dbutilRoot
    log('INFO', 'Extracting ' + tarFileName + ' to ' + pa.dbutilRoot)
    cmd = 'tar -xf ' + tarFilePath + ' -C ' + pa.dbutilRoot
    status, result = executeCommand(cmd)
    if status != 0:
        log('DEBUG', 'ERROR: Failed to extract tar: ' + tarFilePath)
        sys.exit(1)
    log('INFO', 'Extraction successful')

    # Step 4: Verify extracted directory exists
    if not os.path.exists(extractedDir):
        log('DEBUG', 'ERROR: Expected directory not found after extraction: ' + extractedDir)
        sys.exit(1)
    log('INFO', 'Verified extracted directory: ' + extractedDir)

    # Step 5: Create symlink — remove existing first then create fresh
    log('INFO', 'Creating symlink: ' + dbUtilsLink + ' -> ' + extractedDir)
    if os.path.islink(dbUtilsLink):
        log('INFO', 'Removing existing symlink: ' + dbUtilsLink)
        os.unlink(dbUtilsLink)
    cmd = 'ln -s ' + extractedDir + ' ' + dbUtilsLink
    executeCommand(cmd)

    log('INFO', '--- SFS DB Utils Deployment Completed Successfully ---')


# ─────────────────────────────────────────────
# Validate  (aligned with reference validate())
# ─────────────────────────────────────────────

def validate():
    log('INFO', 'Validation code we will add in later iteration or future enhancement')


# ─────────────────────────────────────────────
# Init  (aligned with reference init())
# ─────────────────────────────────────────────

def init():
    """
    Parse CLI arguments and load envInfo global.
    Mirrors reference code init() pattern.
    """
    global pa, envInfo

    parser = argparse.ArgumentParser(description='SFS Deployment')

    # Required arguments
    required = parser.add_argument_group('required arguments')
    required.add_argument('--action',    nargs='?', help='Action (deploy/validate)',
                          required=True,  dest='action')
    required.add_argument('--release',   nargs='?', help='SFS Release version e.g. 0.1.0',
                          required=True,  dest='release')
    required.add_argument('--type',      nargs='?', help='Deployment type: core / config / all / dbutils',
                          required=True,  dest='type')

    # Conditional arguments
    conditional = parser.add_argument_group('conditional arguments')
    conditional.add_argument('--applRoot',  nargs='?',
                             help='Application Root / Deployment Home Directory (required for core/config/all)',
                             required=False, dest='applRoot')
    conditional.add_argument('--envFolder', nargs='?',
                             help='Path to directory containing envInfo.json (required for config/all)',
                             required=False, dest='envFolder')
    conditional.add_argument('--dbutilRoot', nargs='?',
                             help='DB Utils deployment root directory (default: /cls/dbutils) (for dbutils)',
                             required=False, dest='dbutilRoot')

    # Optional arguments
    optional = parser.add_argument_group('optional arguments')
    optional.add_argument('--component', '--list', nargs='+',
                          help='Component name(s): tfg, pct, su or all (default: all)',
                          required=False, dest='component')
    optional.add_argument('--releasePath', nargs='?',
                          help='Path to directory containing tar artifact (default: /cls/release)',
                          required=False, dest='releasePath')

    pa = parser.parse_args()

    # Defaults
    if pa.component is None:
        pa.component = ['all']

    # releasePath defaults to /cls/release if not provided
    if pa.releasePath is None:
        pa.releasePath = '/cls/release'
    elif pa.releasePath[-1:] == '/':
        pa.releasePath = pa.releasePath[:-1]

    # dbutilRoot defaults to /cls/dbutils if not provided
    if pa.dbutilRoot is None:
        pa.dbutilRoot = '/cls/dbutils'
    elif pa.dbutilRoot[-1:] == '/':
        pa.dbutilRoot = pa.dbutilRoot[:-1]

    # applRoot — only required for core/config/all
    if pa.type.lower() in ['core', 'config', 'all']:
        if pa.applRoot is None:
            log('DEBUG', 'ERROR: --applRoot is required for --type ' + pa.type)
            sys.exit(1)
        # Trailing slash normalisation
        if pa.applRoot[-1:] != '/':
            pa.applRoot = pa.applRoot + '/'
        # envFolder defaults to applRoot/env if not provided
        if pa.envFolder is None:
            pa.envFolder = pa.applRoot + 'env'
        elif pa.envFolder[-1:] == '/':
            pa.envFolder = pa.envFolder[:-1]

    log('INFO', '--- SFS Deploy Arguments ---')
    log('INFO', 'action      : ' + pa.action)
    log('INFO', 'release     : ' + pa.release)
    log('INFO', 'type        : ' + pa.type)
    log('INFO', 'applRoot    : ' + str(pa.applRoot))
    log('INFO', 'envFolder   : ' + str(pa.envFolder))
    log('INFO', 'releasePath : ' + pa.releasePath)
    log('INFO', 'dbutilRoot  : ' + pa.dbutilRoot)
    log('INFO', 'component   : ' + str(pa.component))

    # Verify applRoot exists — only for core/config/all
    if pa.type.lower() in ['core', 'config', 'all']:
        checkFileExists(pa.applRoot)

    # Load envInfo.json only when needed i.e. config or all deploy
    if pa.type.lower() in ['config', 'all']:
        envInfo = readJsonData(pa.envFolder + '/envInfo.json')


# ─────────────────────────────────────────────
# Main Entry Point  (aligned with reference main())
# ─────────────────────────────────────────────

def main():
    global logFile
    logFile = setupLogging()
    console('INFO: SFS Deployment started')
    log('INFO', 'Begin: Deploying SFS')

    init()

    if pa.action.lower() == 'deploy':

        deployType = pa.type.lower()

        if deployType == 'core':
            console('INFO: Core deployment started')
            sfsDeploy()
            console('INFO: Core deployment completed successfully')

        elif deployType == 'config':
            console('INFO: Config deployment started')
            sfsConfigDeploy()
            console('INFO: Config deployment completed successfully')

        elif deployType == 'all':
            console('INFO: Core deployment started')
            sfsDeploy()
            console('INFO: Core deployment completed successfully')
            console('INFO: Config deployment started')
            sfsConfigDeploy()
            console('INFO: Config deployment completed successfully')

        elif deployType == 'dbutils':
            console('INFO: DB Utils deployment started')
            sfsDbUtilsDeploy()
            console('INFO: DB Utils deployment completed successfully')

        else:
            console('ERROR: Deployment failed — Invalid type: ' + pa.type
                    + ' — check log: ' + logFile)
            log('DEBUG', 'ERROR: Invalid type: ' + pa.type
                + '. Valid values are: core, config, all, dbutils')
            sys.exit(1)

    elif pa.action.lower() == 'validate':
        validate()

    else:
        console('ERROR: Deployment failed — Invalid action: ' + pa.action
                + ' — check log: ' + logFile)
        log('ERROR', 'Invalid action: ' + pa.action
            + '. Valid values are: deploy, validate')
        sys.exit(1)


main()
